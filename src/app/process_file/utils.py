...
def calculate_item_stats(model: Any, query: str, items: List[str|int]):
    prices = []
    print('-----------------------------------------------------')
    print(f'For {query}:')
    ind = 0
    for name, price in items:
        prob = model.get_similarity_probs(query, name)
        if prob >= THRESHOLD:
...
def process_table(df: pd.DataFrame):
    ...
    for index, row in df.iterrows():
        ...
        stats = calculate_item_stats(model,row['product_name'], finded_items)
